...
class CNN_train():
    def __init__(self, dataset_name, validation=True, verbose=True, imgSize=32, batchsize=128):
        ...
        if dataset_name == 'cifar10' or dataset_name == 'svhn':
            ...
            exit(1)
        if self.validation:
            self.dataloader, self.test_dataloader = get_train_valid_loader(data_dir='data', batch_size=self.batchsize,
                                                                                   augment=True, random_seed=2018,
                                                                                   num_workers=1, pin_memory=True, dataset=dataset_name)
        else:
            if dataset_name == 'cifar10':
                ...
                train_dataset = dset.SVHN(root='data', split='train', download=True,
                        transform=transforms.Compose([
                        transforms.RandomHorizontalFlip(),
                        transforms.Resize(self.imgSize),
                        transforms.ToTensor(),
                ...
                test_dataset = dset.SVHN(root='data', split='test', download=True,
                        transform=transforms.Compose([
                        transforms.Resize(self.imgSize),
                        transforms.ToTensor(),
                        transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225)),
